...
int main(int argc, char **argv) // int argc, char **argv
{
    ...
    data.readSingleFile(argv[1]);
    // data.readSingleFile("/Users/tranlam/development/repo/SummerProject2023/Local-Search/shortest-path/10_nodes/t10_01_data.txt");
    ...
    vector<int> nonZeroRowCount(data.d.size(), data.d.size() - 1);
    vector<int> nonZeroColCount(data.d.size(), data.d.size() - 1);
    ...
    for (size_t i = 1; i < data.d.size(); i++)
    {
        for (size_t j = 1; j < data.d[i].size(); j++)
        {
            if (dis(gen) < sparsity)
            {
